(() => {
  ...
  function readProductForm(form) {
    ...
    const standardSizes = [...form.querySelectorAll('[data-standard-size-label]')].map((input) => {
      const index = Number(input.dataset.index);
      const stored = editorProduct.sizes?.[index];
      return {
        id: stored?.id || uid('size'),
        label: input.value.trim(),
        price: number(form.querySelector(`[data-standard-size-price="${index}"]`)?.value)
      };
    ...
    const readOption = (id) => {
      ...
      return {
        ...
        basePrice: sizes.length ? number(sizes[0].price) : existingOptions[id].basePrice,
        sizes,
    ...
    // Serving choices are entirely optional. When neither switch is enabled,
    // keep this as a regular product and use its own sizes/quantities.
    const usesServingOptions = servingOptions.plain.enabled || servingOptions.rice.enabled;
    const fallback = servingOptions.plain.enabled ? servingOptions.plain : (servingOptions.rice.enabled ? servingOptions.rice : null);
    const keepLegacyRice = !usesServingOptions && !editorProduct.servingOptions && Boolean(editorProduct.requiresRice);
    return {
      ...
      requiresRice: usesServingOptions ? (servingOptions.rice.enabled && !servingOptions.plain.enabled) : keepLegacyRice,
      riceAllowedIds: usesServingOptions ? servingOptions.rice.riceAllowedIds : (keepLegacyRice ? (editorProduct.riceAllowedIds || []) : []),
      sizes: usesServingOptions ? (fallback?.sizes || []) : standardSizes,
      servingOptions: usesServingOptions ? servingOptions : undefined
    };
  }
  ...
  function productServingOptions(product) {
    ...
    return { plain: option('plain'), rice: option('rice') };
  }
  ...
  function renderProductModal() {
    ...
    const standardSizeRows = (p.sizes || []).map((size, index) => `<div class="mini-row"><input data-standard-size-label data-index="${index}" value="${esc(size.label)}" placeholder="اسم الحجم أو الكمية"><input class="short" data-standard-size-price="${index}" type="number" min="0" step="0.01" value="${number(size.price)}" placeholder="السعر"><button type="button" data-action="remove-standard-size" data-index="${index}">×</button></div>`).join('');
    const standardEditor = `<section class="serving-option-editor"><div class="size-head"><div><h3>صنف عادي بلا خيارات تقديم</h3><p class="muted">استخدم هذا القسم للأصناف ذات النوع الأساسي الواحد. أضف أحجامها أو كمياتها وأسعارها هنا، واترك «سادة» و«مع الرز» مقفلين.</p></div><button class="secondary" type="button" data-action="add-standard-size">＋ إضافة حجم أو كمية</button></div><div class="mini-list">${standardSizeRows || '<p class="muted">لا توجد أحجام أو كميات مضافة. سيُستخدم السعر القياسي أعلاه إن لم تضف أيًّا منها.</p>'}</div></section>`;
    const optionEditor = (id, label, rice) => {
      ...
      return `<section class="serving-option-editor"><div class="switch-row"><div><strong>${label}</strong><small>${rice ? 'يعرض نوع الرز بعد اختياره.' : 'لا يعرض خيارات الرز للعميل.'}</small></div><input class="switch" name="servingEnabled-${id}" type="checkbox" ${option.enabled ? 'checked' : ''}></div>${riceChoices}<div class="field full"><div class="size-head"><h4>أحجام / كميات ${label}</h4><button class="secondary" type="button" data-action="add-serving-size" data-serving="${id}">＋ إضافة حجم أو كمية</button></div><p class="muted">أضف الحجم أو الكمية وسعرها؛ لا يظهر أي سعر في زر «${label}» نفسه.</p><div class="mini-list">${sizeRows || '<p class="muted">لا توجد أحجام أو كميات مضافة.</p>'}</div></div></section>`;
    };
    openModal(`<div class="modal-head"><h2>${p.name ? 'تعديل صنف' : 'إضافة صنف جديد'}</h2><button class="modal-close" data-action="close-modal" aria-label="إغلاق">×</button></div><form id="productForm" class="field-grid"><label class="field">اسم الصنف<input name="name" required value="${esc(p.name)}"></label><label class="field">القسم<select name="category">${state.categories.map((category) => `<option value="${esc(category.id)}" ${p.category === category.id ? 'selected' : ''}>${esc(category.name)}</option>`).join('')}</select></label><label class="field full">الوصف<input name="description" value="${esc(p.description)}" placeholder="وصف مختصر يظهر للعميل"></label><label class="field">السعر القياسي (لصنف بلا خيارات تقديم)<input name="basePrice" type="number" required min="0" step="0.01" value="${number(p.basePrice)}"></label><label class="field">السعر قبل التخفيض (اختياري)<input name="oldPrice" type="number" min="0" step="0.01" value="${number(p.oldPrice)}"></label><label class="field">السعرات (اختياري)<input name="calories" type="number" min="0" step="1" value="${number(p.calories)}"></label><label class="field">شارة أعلى الصورة (اختياري)<input name="badge" value="${esc(p.badge)}" placeholder="مثال: تحضير 25 دقيقة"></label><label class="field full">رابط أو مسار الصورة<input name="image" required value="${esc(p.image)}" placeholder="images/product.webp أو https://..."></label><label class="field">رفع صورة من الجهاز<input id="productImageFile" type="file" accept="image/*"><small class="muted">ترفع الصورة إلى ImageKit عند اختيارها.</small></label><div class="field"><span>معاينة الصورة</span><img class="image-preview" src="${esc(p.image)}" alt="معاينة"></div><label class="field"><span>الحالة</span><div class="switch-row"><small>عرض الصنف في الموقع</small><input class="switch" name="active" type="checkbox" ${p.active ? 'checked' : ''}></div></label><div class="field full">${standardEditor}<h3>خيارات تقديم الصنف (اختياري)</h3><p class="muted">فعّل سادة أو مع الرز أو الاثنين فقط عندما يحتاج العميل لاختيار طريقة التقديم أولًا. العميل يرى الأسعار مع الأحجام أو الكميات فقط، وليس في زر «سادة» أو «مع الرز» نفسه.</p>${optionEditor('plain', 'سادة', false)}${optionEditor('rice', 'مع الرز', true)}</div><div class="form-footer field full"><button class="secondary" type="button" data-action="close-modal">إلغاء</button><button class="primary" type="submit">حفظ الصنف</button></div></form>`);
  }

  function openCouponEditor(coupon) {
    const announcementDefaults = { eyebrow: 'عرض خاص لك', title: 'وفّر في طلبك اليوم', discountLabel: 'خصم', codeLabel: 'كود العرض', copyButton: 'نسخ الكود', scopeLabel: 'يسري على', allScopeText: 'كل أصناف القائمة', methodsLabel: 'متاح مع', allMethodsText: 'كل طرق الاستلام', termsTitle: 'الشروط والأحكام', terms: ['يُستخدم الكود مرة واحدة لكل عميل.', 'لا يُجمع مع عروض أو أكواد خصم أخرى.'] };
    editorCoupon = copy(coupon || { id: uid('coupon'), code: '', type: 'percent', amount: 10, minimum: 0, active: true, scope: 'all', productIds: [], methodIds: [], announce: true, singleUse: true, announcementText: announcementDefaults });
    const c = editorCoupon;
    const existingText = c.announcementText && typeof c.announcementText === 'object' ? c.announcementText : {};
    const announcementText = { ...announcementDefaults, ...existingText };
    announcementText.terms = (Array.isArray(existingText.terms) ? existingText.terms : String(existingText.terms || '').split(/\r?\n/)).map((term) => String(term || '').trim()).filter(Boolean);
    ...
    openModal(`<div class="modal-head"><h2>${c.code ? 'تعديل كود الخصم' : 'إضافة كود خصم'}</h2><button class="modal-close" data-action="close-modal">×</button></div><form id="couponForm" class="field-grid"><label class="field">الكود<input name="code" required maxlength="32" value="${esc(c.code)}" placeholder="مثال: WELCOME10"></label><label class="field">نوع الخصم<select name="type"><option value="percent" ${c.type === 'percent' ? 'selected' : ''}>نسبة مئوية</option><option value="fixed" ${c.type === 'fixed' ? 'selected' : ''}>مبلغ ثابت</option></select></label><label class="field">قيمة الخصم<input name="amount" type="number" required min="0" step="0.5" value="${number(c.amount)}"></label><label class="field">الحد الأدنى للأصناف المشمولة<input name="minimum" type="number" min="0" step="0.5" value="${number(c.minimum)}"></label><label class="field full"><span>نطاق الخصم</span><select name="scope"><option value="all" ${c.scope !== 'products' ? 'selected' : ''}>كل الأصناف</option><option value="products" ${c.scope === 'products' ? 'selected' : ''}>أصناف محددة فقط</option></select><small class="muted">عند تحديد أصناف: يطبق الخصم على الأصناف المختارة فقط، ولا يدخل أي صنف آخر في قيمة الخصم.</small></label><div class="field full"><span>الأصناف المشمولة عند اختيار «أصناف محددة»</span><div class="check-grid coupon-check-grid">${productChoices}</div></div><div class="field full"><span>طرق الاستلام المسموحة</span><div class="check-grid coupon-check-grid">${methodChoices}</div><small class="muted">اتركها بلا اختيار ليعمل الكود مع كل طرق الاستلام. رسوم التوصيل لا تُخصم دائمًا.</small></div><section class="coupon-copy-editor field full"><div class="coupon-copy-editor-title"><strong>نصوص إعلان الكود</strong><small>كل هذه العبارات تظهر للعميل عند فتح الموقع ويمكن تخصيصها لكل كود بشكل مستقل.</small></div><div class="coupon-copy-grid"><label class="field">العبارة الصغيرة أعلى الإعلان<input name="announcementEyebrow" value="${esc(announcementText.eyebrow)}"></label><label class="field">العنوان الرئيسي<input name="announcementTitle" value="${esc(announcementText.title)}"></label><label class="field">كلمة قبل قيمة الخصم<input name="announcementDiscountLabel" value="${esc(announcementText.discountLabel)}"></label><label class="field">عنوان خانة الكود<input name="announcementCodeLabel" value="${esc(announcementText.codeLabel)}"></label><label class="field">نص زر النسخ<input name="announcementCopyButton" value="${esc(announcementText.copyButton)}"></label><label class="field">عنوان نطاق الخصم<input name="announcementScopeLabel" value="${esc(announcementText.scopeLabel)}" placeholder="مثال: يسري على"></label><label class="field">نص النطاق عند «كل الأصناف»<input name="announcementAllScopeText" value="${esc(announcementText.allScopeText)}" placeholder="مثال: كل أصناف القائمة"></label><label class="field">عنوان طرق الاستلام<input name="announcementMethodsLabel" value="${esc(announcementText.methodsLabel)}" placeholder="مثال: متاح مع"></label><label class="field full">نص الطرق عند اختيار كل الطرق<input name="announcementAllMethodsText" value="${esc(announcementText.allMethodsText)}" placeholder="مثال: كل طرق الاستلام"></label></div><label class="field full">عنوان قسم الشروط والأحكام<input name="announcementTermsTitle" value="${esc(announcementText.termsTitle)}" placeholder="مثال: الشروط والأحكام"></label><label class="field full">الشروط والأحكام<textarea name="announcementTerms" rows="5" placeholder="اكتب كل شرط في سطر مستقل — سيظهر بجانبه الرمز • للعميل">${esc(announcementText.terms.join('\n'))}</textarea><small class="muted">كل سطر شرط منفصل. اترك الحقل فارغًا لإخفاء قسم الشروط من الإعلان.</small></label></section><div class="field full"><div class="switch-row"><div><strong>إظهار إعلان الكود عند فتح الموقع</strong><small>يعرض الكود وزر نسخه ونطاقه وشروطه للعميل فور فتح الموقع.</small></div><input class="switch" name="announce" type="checkbox" ${c.announce !== false ? 'checked' : ''}></div><div class="switch-row"><div><strong>الحالة</strong><small>الكود المفعّل يمنع استخدامه أكثر من مرة لنفس الجوال أو الجهاز أو IP.</small></div><input class="switch" name="active" type="checkbox" ${c.active ? 'checked' : ''}></div></div><div class="form-footer field full"><button class="secondary" type="button" data-action="close-modal">إلغاء</button><button class="primary" type="submit">حفظ الكود</button></div></form>`);
  }
  ...
  document.addEventListener('click', (event) => {
    ...
    if (action === 'add-standard-size') { captureProductDraft(); (editorProduct.sizes || (editorProduct.sizes = [])).push({ id: uid('size'), label: '', price: 0 }); renderProductModal(); }
    if (action === 'remove-standard-size') { captureProductDraft(); editorProduct.sizes?.splice(Number(index), 1); renderProductModal(); }
  ...
  document.addEventListener('submit', async (event) => {
    ...
    if (form.id === 'couponForm') {
      ...
      const announcementText = {
        eyebrow: value.announcementEyebrow.trim(),
        title: value.announcementTitle.trim(),
        discountLabel: value.announcementDiscountLabel.trim(),
        codeLabel: value.announcementCodeLabel.trim(),
        copyButton: value.announcementCopyButton.trim(),
        scopeLabel: value.announcementScopeLabel.trim(),
        allScopeText: value.announcementAllScopeText.trim(),
        methodsLabel: value.announcementMethodsLabel.trim(),
        allMethodsText: value.announcementAllMethodsText.trim(),
        termsTitle: value.announcementTermsTitle.trim(),
        terms: value.announcementTerms.split(/\r?\n/).map((term) => term.trim()).filter(Boolean)
      };
      Object.assign(editorCoupon, { code: value.code.trim().toUpperCase(), type: value.type, amount: number(value.amount), minimum: number(value.minimum), scope: value.scope === 'products' ? 'products' : 'all', productIds, methodIds, announce: form.elements.announce.checked, active: form.elements.active.checked, singleUse: true, announcementText });
