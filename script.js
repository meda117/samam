  ...
  function renderProduct(product) {
    ...
    return `<article class="product-card" data-product-id="${esc(product.id)}">
      ...
      <div class="product-body"><div><p class="product-category">${esc(categoryName(product.category))}</p><h3>${esc(product.name)}</h3><p class="product-description">${esc(product.description || '')}</p></div>
       ...
       <div class="product-bottom"><div><span class="product-price">${needsServing ? 'اختر سادة أو مع الرز' : (needsSize ? 'اختر الحجم أولًا' : (missingRiceType ? 'لا يوجد نوع رز متاح' : money(price, state.business.currency)))}</span>${!needsServing && !needsSize && Number(product.oldPrice) > price ? `<del>${money(product.oldPrice, state.business.currency)}</del>` : ''}</div>
       ...
       <button class="add-button" data-action="add-cart" data-product-id="${esc(product.id)}" ${cardDisabled ? 'disabled' : ''}>${cardDisabled ? (needsServing ? 'اختر سادة أو مع الرز أولًا' : (needsSize ? 'اختر الحجم أو الكمية أولًا' : (missingRiceType ? 'لا يوجد نوع رز متاح' : 'اختر نوع الرز أولًا'))) : 'أضف إلى السلة'}</button></div>
    </article>`;
  }
